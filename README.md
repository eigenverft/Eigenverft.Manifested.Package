...
new PowerShell session as instructed. Its final result uses the standard timestamped status format;
encoded Eigenverft versions also show their build time in UTC without requiring `-Verbose`.
...
`Invoke-PackageDepotMaterialize -AllTrusted` materializes every already signed-and-trusted definition available for the current platform, including its dependencies and complete artifact file set. It requires confirmation, never adds catalog trust, and never prunes depot files. Review with `-WhatIf` first: a trusted catalog can include multi-gigabyte runtimes or models. Use `-PublisherId`, `-Tag`, or `-ExcludeDefinitionId` to narrow the run.
...
Later runs reuse files from the depot when the engine finds a matching artifact under `PackageDepot\{depotNamespace}\{definitionId}\{releaseTrack}\{version}\...`. The namespace comes from the selected definition and keeps publication groups from colliding; the shipped Eigenverft definitions use `evf`, while an omitted or empty value uses `default`. You only need `Add-TeamPackageDepot` again if that Windows user profile has not been configured yet.
