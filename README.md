...
[![PowerShell Support](https://img.shields.io/badge/PowerShell-5.1%2B%20Desktop%2FCore-5391FE?logo=powershell&logoColor=white)](#requirements) [![Build Status](https://img.shields.io/github/actions/workflow/status/eigenverft/Eigenverft.Manifested.Package/cicd.yml?branch=main&label=build)](https://github.com/eigenverft/Eigenverft.Manifested.Package/actions/workflows/cicd.yml) [![License](https://img.shields.io/github/license/eigenverft/Eigenverft.Manifested.Package?logo=mit)](LICENSE)

Eigenverft.Manifested.Package is a Windows-focused PowerShell package engine for repeatable developer-machine setup. It uses explicit package-definition JSON, trusted publishers, configurable package endpoints, reusable depots, and local inventory so a toolchain can be assigned, rerun, audited, and removed with predictable behavior.
...
- Core development-tool definitions for runtimes, CLIs, editors, archives, SDKs, and prerequisites
...
This path intentionally bypasses TLS certificate validation for the bootstrap download and initial PSGallery install. Prefer normal `Install-Module` when the machine trust chain works.
