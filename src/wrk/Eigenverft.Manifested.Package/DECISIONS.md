...
| Trusted catalog materialize | Public `Invoke-PackageDepotMaterialize` materializes deduplicated `signedTrusted` current-platform definitions by default without a normal confirmation prompt; `-WhatIf` previews, `-AcceptUnknownSigningKey` explicitly opts into signed unknown-key trust for selected roots, unsigned and blocked root definitions remain excluded while dependency trust follows the existing package policy, failures continue by default, `-FailFast` stops early, and depot files are never pruned. |
