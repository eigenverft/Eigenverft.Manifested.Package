...
- **`previousByVersion` today:** usable via **`Invoke-Package -PackageVersion previousByVersion`** (tests in `Package.Definitions.Tests.ps1`); not authorable on the definition target row.
...
- **Post-selection install policy:** `versionUpdatePolicy.onNewSelectedVersion` (`replacePackageOwnedInstall` / `fail`) governs **install slot** behavior after the selected version **changes** - not which versions are eligible (`Package.OwnershipLifecycle.Tests.ps1`).
