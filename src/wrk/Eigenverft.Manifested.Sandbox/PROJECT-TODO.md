...
- **Feature issue: stop early when requested packages disagree on dependencies.** As a user installing multiple packages in one command, I want dependency conflicts reported before any install starts, so I can change the package set instead of debugging a half-finished run.
  Requester perspective: sandbox profile author, team onboarding maintainer, release engineer.
...
- **Feature issue: keep the depot clean for file shares and NAS sync.** As a user syncing a package depot through a file share or NAS, I want the depot to contain only files that are meant to be shared, so sync tools do not copy temp files, sidecar files, or half-built package state.
  Requester perspective: home NAS user, team share maintainer, offline sandbox user.
  Outcome: depot hydration, mirror, and reuse behavior remains predictable when files are copied by external sync tools.

- **Feature issue: show which package types really work offline.** As an offline or low-network operator, I want each package type to say whether it can install fully from the depot, so I can build a sandbox profile that works after the depot is seeded.
  Requester perspective: air-gapped lab user, Windows Sandbox power user, team laptop bootstrap owner.
...
- **Feature issue: search packages by name or command.** As a user exploring available tools, I want to find packages by friendly name, command, tag, or publisher without knowing the exact `DefinitionId`, so shipped and team endpoint packages are easier to discover.
  Requester perspective: first-time sandbox user, team member using a shared endpoint, demo operator.
...
- **Feature issue: show package state without reading JSON.** As a day-to-day package user, I want package state and recent operations shown as readable tables and summaries, so I do not need to inspect inventory files for normal troubleshooting.
  Requester perspective: sandbox user, support helper, team onboarding maintainer.
...
- **Feature issue: explain packages that can live side by side.** As a runtime package maintainer, I want guidance for packages whose major versions can coexist, so definitions like .NET SDK 9 and .NET SDK 10 do not look like accidental duplicates.
  Requester perspective: .NET developer, package catalog maintainer, sandbox profile author.
...
These are useful stories, but they belong to a separate manager or orchestration product rather than the sandbox-first module:
...
- **Manager story: save and reuse named package profiles.** As a team or enterprise operator, I want to export and reapply named package sets, so repeated sandbox or workstation setup does not depend on hand-maintained command strings.
...
Sandbox already covers per-machine **Assigned** state, endpoint-driven definitions, `versionUpdatePolicy` on re-invoke, and flexible **team/web endpoint / depot / publisher** layout. Catalog growth at scale is expected to be driven by LLM-maintained JSON on endpoints, including an Eigenverft online endpoint and team endpoints, not by turning the sandbox module into a fleet manager.

## Review / Questions

- Should the "wait before auto-picking a new release" rule be global, package-specific, publisher-specific, or only enabled for sensitive packages?
...
- [P3] Tooling - 2026-05-23: Bumped package-definition schema to 1.6 (`schema update`) for shipped definitions including SevenZip and DotNetSdk10.
- [P3] Tooling - 2026-05-23: Added managed **SevenZip** package definition (pinned 7-Zip MSI, `7z` on PATH).
- [P3] Tooling - 2026-05-23: Added managed **DotNetSdk10** package definition (portable .NET 10 SDK, `dotnet` on PATH). README documents bootstrap and NAS depot reuse.
- [P3] Tooling - 2026-05-22: Fixed npm materialization to use lock entry `name` when alias folder differs from registry package (Codex `@openai/codex@…` platform versions).
- [P3] Tooling - 2026-05-22: Resolved materialized npm installs from local tarballs instead of live registry installs during assign.
- [P2] Tooling - 2026-05-22: Added package version selection (`latestByVersion`, pins) and refreshed shipped definition versions.
- [P2] Tooling - 2026-05-21: Retired online npm package installs; npm CLIs use materialized tarball workflow under package-owned layout.
- [P3] Tooling - 2026-05-20: OpenCode CLI depot integration and npm tarball materialization hardening (registry pack, depot mirror).
- [P2] Tooling - 2026-05-17: Added `powershellModuleInstaller` for staged Gallery nupkg installs (Eigenverft.Manifested.Agent path); PS module bootstrap and hidden child-console fixes.
- [P2] Architecture - 2026-05-16: Retired legacy package-definition schemas; quiet expected readiness probes; PkgRepos → endpoint naming cleanup.
- [P2] Tooling - 2026-05-16: Extended default user story (team depot/endpoint/publisher commands, `Get-PackageState`, shipped Eigenverft defaults).
- [P1] Trust - 2026-05-16: Package publisher trust commands and inventory management (`Add-TeamPackagePublisher`, depot/endpoint helpers).
- [P2] Architecture - 2026-05-15: Publisher and endpoint inventory cleanup; removed retired repository terminology from commands and config.
- [P2] Architecture - 2026-05-14: Replaced package repository inventory with **endpoint** inventory; definitions discovered by endpoint scan order.
- [P2] Architecture - 2026-05-13: Decoupled package definition resolution from repository pins; publisher trust drives definition authority.
- [P2] Tooling - 2026-05-11: First package depot management (`Add-TeamPackageDepot`, mirror reconcile); renamed package state files (`PackageAssignmentInventory`, `PackageOperationHistory`).
- [P3] Tooling - 2026-05-10: Added **CursorCli** / EigenverftModule package path; major command surface cleanup; `Get-SandboxVersion` Invoke-Package examples.
- [P3] Tooling - 2026-05-10: **VSCodeUser** definition, module self-update path, restored `Invoke-WebRequestEx` and `Initialize-ProxyAccessProfile` as public surface.
- [P2] Architecture - 2026-05-10: Archive abstraction in ExecutionCore; shortened install path layout (`Installed` → `Inst`, staging → `InstStage` / `FileStage`).
- [P3] Tooling - 2026-05-09: Managed **Python** runtime working on schema 1.3+ wire path.
- [P2] Architecture - 2026-05-05: Package wire 1.1 naming refactor; split config/install; ExecutionCore + Package subfolder layout; central module loads.
- [P2] Tooling - 2026-05-05: `Invoke-Package` batch **FailFast** option.
- [P2] Tooling - 2026-05-04: Package command shims and ownership metadata.
- [P2] Tooling - 2026-05-03: Package assignment inventory and append-only operation history.
- [P2] Tooling - 2026-05-02: Generic package command surface (`Invoke-Package`, depot, readiness, removal groundwork).
- [P3] Tooling - 2026-04-30: Notepad++ installer package; hardened package execution and trust metadata.
- [P2] Architecture - 2026-04-27: Package file staging, depot layout, and directory management under `Evf.Sandbox`.
- [P3] Tooling - 2026-04-27: Qwen GGUF model package (pinned variant) and llama.cpp-related assets.
- [P2] Architecture - 2026-04-25: Consolidated package runtime model; migrated Python/Node/Git/VS Code runtimes to PackageModel.
- [P3] Tooling - 2026-04-24: Managed **PowerShell 7** and **VS Code** runtime packages on PackageModel path.
- [P2] Architecture - 2026-04-19: First working PackageModel install path (VS Code runtime).
- [P2] Bootstrap - 2026-04-13: **Invoke-WebRequestEx** compatibility, proxy defaults, and download resilience for sandbox bootstrap.
...
- [P3] Tooling - 2026-03-19: Added managed **OpenCode**, **Qwen**, and **Gemini** CLI npm runtimes; explicit managed npm proxy ownership.
- [P3] Tooling - 2026-03-19: **OpenCode** runtime bootstrap in sandbox metadata.
- [P3] Tooling - 2026-03-14: Managed **GitHub CLI** (`gh`) runtime.
- [P3] Tooling - 2026-03-14: **Codex** CLI npm runtime; shared temp staging; VC++ prerequisite before Codex assign.
...
- [P2] Bootstrap - 2026-03-13: Windows Sandbox `.wsb` profile, embedded proxy/bootstrap chain, and `Get-SandboxVersion`.
- [P2] Bootstrap - 2026-03-12: Initial **Eigenverft.Manifested.Sandbox** module, CI workflow, and sandbox-first repo layout.
