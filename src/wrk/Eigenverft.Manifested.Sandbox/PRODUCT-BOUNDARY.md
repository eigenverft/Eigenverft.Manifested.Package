...
Eigenverft.Manifested.Sandbox is a Windows Sandbox first bootstrap and package-assignment module.

Its job is to make a fresh Windows Sandbox, and secondarily a normal Windows user profile, quickly become a useful development environment through trusted package definitions, local state, reusable depots, and explicit package assignment.

The product should feel like a small, reliable package engine for one machine or one sandbox session, not like a central fleet manager.
...
- A developer who wants a fresh Windows Sandbox to become useful in minutes.
...
- Schema changes are allowed when the product is still shaping itself, but the schema should become simpler and clearer over time.
...
- Keep the default mental model local: one sandbox, one user profile, one package assignment inventory.
...
- Makes a sandbox or user profile more repeatable.
...
These may become a separate manager or orchestration product, but should not define the sandbox module:
...
- Strong for Windows Sandbox, team dev machines, internal package shares, and isolated networks.
...
- Does this make one sandbox or one user profile easier to prepare?
